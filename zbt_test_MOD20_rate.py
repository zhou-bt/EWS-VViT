...
from pack.ConfusionMatrix_rate import ConfusionMatrix
import torch
...
# from model.my_vit_8f_model import vit_base_patch16_224_in21k as create_model
...
def main(args):
    ...
    with torch.no_grad():
        for epoch in range(args.epochs):
            ...
            for step, (images, labels, index) in enumerate(val_loader):
                ...
                video_labels.append(labels.to("cpu").numpy())
                index.to(device)
                images = [images, index]
                sample_num += batch_size
                pred = model(images)
                pred_classes = torch.max(pred, dim=1)[1]
                video_pred.append(pred_classes.to("cpu").numpy())
                accu_num += torch.eq(pred_classes, labels.to(device)).sum()
                val_loader.desc = "[valid epoch {}],{},{},acc: {:.3f}".format(epoch, accu_num.item(),sample_num,
                                                                           accu_num.item() / sample_num)

    #congfusion matrix
    root_path = '/data/zhoubotong/dataset/MOD20_frame_1/'
    labels = []
    ...
    save_path = os.path.join(root_path, 'confusion_matrix_MOD20')
    if not os.path.exists(save_path):
        os.mkdir(save_path)
    plt.savefig(os.path.join(save_path, 'MOD20'), format='png')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='/data/zhoubotong/project/0_mframe_input_module/weights/MOD20_lr0.00025_batch1_vit_base_patch16_224_in21k.pth/model-best_95.3.pth',
                        help='initial weights path')
    # 数据集所在根目录
    parser.add_argument('--dataset_name', default='MOD20', help='create model name')
