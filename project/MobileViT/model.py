...
class MobileViT(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        # [bt, 3, 224, 224]
        x = self.conv_1(x)  #[bt, 16, 112, 112]
        x = self.layer_1(x) #[bt, 16, 112, 112]
        x = self.layer_2(x) #[bt, 24, 56, 56]

        x = self.layer_3(x) #[bt, 48, 28, 28]
        x = self.layer_4(x) #[bt, 64, 14, 14]
        x = self.layer_5(x) #[bt, 8, 7, 7]
        x = self.conv_1x1_exp(x)    #[bt, 320, 7, 7]

        x = self.classifier(x)
        x = rearrange(x, '(b t) c -> b t c', b=8, t=8)
        x = x.mean(1)
        return x
