...
class FSATransformerEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for sp_attn, temp_attn, ff in self.layers:
            ...
            sp_attn_x = sp_attn_x.chunk(b, dim=0)      #tuple:2{tensor(1,196,768)}
            sp_attn_x = [temp[None] for temp in sp_attn_x]      #list:2{tensor(1,196,768)}
            sp_attn_x = torch.cat(sp_attn_x, dim=0).transpose(1, 2)      #(2,196,1,768)
            sp_attn_x = torch.flatten(sp_attn_x, start_dim=0, end_dim=1)    #(392,1,768)
            #(392,1,768)
            temp_attn_x = temp_attn(sp_attn_x) + sp_attn_x  # Temporal attention

            x = ff(temp_attn_x) + temp_attn_x  # MLP    #(392,1,768)

            # Again reshape tensor for spatial attention
            x = x.chunk(b, dim=0)   #tuple:2{tensor(196,1,768)}
            x = [temp[None] for temp in x]  #list:2{tensor(1,196,1,768)}
            x = torch.cat(x, dim=0).transpose(1, 2)     #(2,1,196,768)
