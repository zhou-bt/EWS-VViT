...
class MViT(nn.Module):
    ...
    def __init__(self, cfg):
        super().__init__()
        # Get parameters.
        assert cfg.train_corp_size == cfg.test_corp_size
        # Prepare input.
        self.batch_size = cfg.batch_size
        self.num_segments = cfg.num_segments
        in_chans = 3
        spatial_size = cfg.train_corp_size #224
        # Prepare output.
        num_classes = cfg.num_classes
        embed_dim = cfg.embed_dim  #96
        # MViT params.
        num_heads = cfg.num_head  #1
        depth = cfg.depth #24
        self.cls_embed_on = cfg.cls_embed_on   #f
        self.use_abs_pos = cfg.use_abs_pos #f
        self.zero_decay_pos_cls = cfg.zero_decay_pos_cls   #f
        ...
        patch_embed = PatchEmbed(
            dim_in=in_chans,
            dim_out=embed_dim,
            kernel=cfg.patch_kernel,  #[7,7]
            stride=cfg.patch_stride,   #[4,4]
            padding=cfg.patch_padding,     #[3,3]
        )
        self.patch_embed = patch_embed

        patch_dims = [  #[56,56]
            spatial_size // cfg.patch_stride[0],    #224/4
            spatial_size // cfg.patch_stride[1],   #224/4
        ]
        num_patches = patch_dims[0]*patch_dims[1]  #56*56

        dpr = [
            x.item() for x in torch.linspace(0, cfg.droppath_rate, depth)   #droppath_rate:0.3
        ...
        if self.use_abs_pos:    #abs postion code
        ...
        for i in range(depth):
            ...
            attention_block = MultiScaleBlock(
                ...
                mode=cfg.mode,    #conv
                has_cls_embed=self.cls_embed_on,    #false
                pool_first=cfg.pool_first,    #false
                rel_pos_spatial=cfg.rel_pos_spatial,   #true
                rel_pos_zero_init=cfg.rel_pos_zero_init,    #false
                residual_pooling=cfg.residual_pooling,    #true
                dim_mul_in_att=cfg.dim_mul_in_att,    #true
            )
        ...
        self.head = TransformerBasicHead(
            embed_dim,
            num_classes,
            dropout_rate=cfg.dropout_rate,   #0.0
            act_func=cfg.act_func,    #str softmax
        )
        if self.use_abs_pos:    #false
            trunc_normal_(self.pos_embed, std=0.02)
        if self.cls_embed_on:    #false
...
def _prepare_mvit_configs(cfg):
    ...
    for i in range(len(cfg.pool_Q_stride)):
        ...
        pool_q[cfg.pool_Q_stride[i][0]] = cfg.pool_kqv_kernel
    ...
    for i in range(len(cfg.pool_kv_stride)):
        ...
        pool_kv[cfg.pool_kv_stride[i][0]] = cfg.pool_kqv_kernel
