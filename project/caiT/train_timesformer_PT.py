...
from full_attention_cls_abs_timeabs import vit_base_patch16_224_in21k as create_model
...
def main(args):
    ...
    val_acc_best = 0
    pre_wigth = args.weights.split('/')[-1]
    ...
    if args.weights != "":
        ...
        weights_dict = torch.load(args.weights, map_location=device)['model_state']
        del_keys = ['model.head.weight', 'model.head.bias']
        for k in del_keys:
            del weights_dict[k]

        weights_dict_new = OrderedDict()
        for k, v in weights_dict.items():
            weights_dict_new[k[6:]] = v

        if 'model' in list(weights_dict_new.keys())[0]:
            model = model.cuda()
            ...
            model.load_state_dict(weights_dict_new, False)
        else:
            model.load_state_dict(weights_dict_new, False)
            model = model.cuda()
            # model = torch.nn.DataParallel(model, device_ids=[x for x in range(len(args.device))])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='/data/zhoubotong/project/0_mframe_input_module/project/TimeSformer/TimeSformer_divST_8x32_224_K600.pyth',
